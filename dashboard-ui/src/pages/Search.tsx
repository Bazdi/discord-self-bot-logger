...
export default function Search() {
  ...
  const parsedQuery = useMemo(
    () => parseDiscordSearchQuery(rawQuery),
    [rawQuery],
  );
    ...
    async (queryText: string, afterCursor?: string | null) => {
      ...
      try {
        ...
        if (parsedQuery.filter)
          params.set("filters", JSON.stringify(parsedQuery.filter));
        params.set("limit", "25");
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                ...
                {!loading &&
                results.length === 0 &&
                (parsedQuery.text || parsedQuery.filter) ? (
                ...
                {!loading && !parsedQuery.text && !parsedQuery.filter ? (
                  <div className="flex items-center gap-2 rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
                    <SearchIcon className="size-4" />
                    Open the top drawer and enter a query like `hello
                    in:1241473215264460885`.
                  </div>
                ) : null}
              </div>
